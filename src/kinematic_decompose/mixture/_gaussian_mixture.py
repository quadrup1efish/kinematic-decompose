...
class GaussianMixture(BaseMixture):
    ...
    def _initialize(self, X, resp, xp=None):
        ...
        if self.precisions_init is None:
            ...
            self.covariances_ = xp.linalg.inv(self.precisions_init)
    ...
    def _m_step(self, X, log_resp, sample_weight=None, xp=None):
        ...
        if sample_weight is not None:
            resp = resp * sample_weight[:, xp.newaxis]
    ...
    def mbic(self, X):
        return self.bic(X)/len(X)

    def icl(self, X):
        _, log_resp = self._e_step(X)
        resp = np.clip(np.exp(log_resp), 1e-12, 1.0)
        entropy = -np.sum(resp * np.log(resp))
        icl = self.bic(X) + 2 * entropy
        return icl
