...
public:
    ...
    */
    ...
    /** Create the potential from particles represented by compact-support cubic-spline
        density kernels. The softening radius is the kernel support radius; it may be
        a scalar shared by all particles or an array with one value per particle.
        Unlike the ``smoothing`` argument of the point-particle factory above, these
        radii have units of length and define the physical source density. */
    static shared_ptr<const Multipole> createSoftened(
        const particles::ParticleArray<coord::PosCyl> &particles,
        const std::vector<double> &softening,
        coord::SymmetryType sym, int lmax, int mmax,
        unsigned int gridSizeR, double rmin = 0., double rmax = 0., bool fixOrder = false);
