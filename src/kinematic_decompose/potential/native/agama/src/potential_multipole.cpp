    ...
    double& rmin, double& rmax)
{
    ...
    size_t nbody = radii.size();
    std::nth_element(radii.begin(), radii.begin() + nbody/2, radii.end());
    double rhalf = radii[nbody/2];
    double spacing = 1 + sqrt(20./gridSizeR);
    int nskip = std::min<int>(std::log(nbody+1)/std::log(2), nbody-1);
    if(rmin==0) {
...
// Accumulate the spherical-harmonic density coefficients of compact-support
// particle kernels directly.  This avoids evaluating the source on the full
// (radius, theta, phi) grid used by the generic BaseDensity path.
inline double softenedKernel(double q)
{
    if(q >= 1)
        return 0;
    return q < 0.5 ?
        8/M_PI * (1 - 6*q*q + 6*q*q*q) :
        16/M_PI * pow_3(1-q);
}

void softenedKernelAngularMoments(double radius, double particleRadius, double h,
    int lmax, std::vector<double>& moments)
{
    moments.assign(lmax+1, 0.);
    // A particle at the origin produces a spherically symmetric kernel.
    if(radius==0 || particleRadius==0) {
        double q = (radius + particleRadius) / h;
        moments[0] = softenedKernel(q) / pow_3(h);
        return;
    }
    double muMin = (pow_2(radius) + pow_2(particleRadius) - pow_2(h)) /
    ...
    // makes the quadrature smooth on both pieces of the cubic spline.
    for(int piece=0; piece<2; piece++) {
        double a = piece==0 ? muMin : std::max(muMin, muHalf);
        double b = piece==0 ? std::min(1., muHalf) : 1.;
        if(b <= a)
            continue;
        for(int n=0; n<order; n++) {
            double mu = a + (b-a) * nodes[n];
            double q = sqrt(std::max(0., pow_2(radius) + pow_2(particleRadius) -
                2 * radius * particleRadius * mu)) / h;
            double value = 0.5 * (b-a) * weights[n] * softenedKernel(q) /
                pow_3(h);
            moments[0] += value;
            if(lmax>0) {
                double p0 = 1, p1 = mu;
    ...
    unsigned int gridSizeR, double rmin, double rmax, bool fixOrder)
{
    ...
    std::vector<double> expandedSoftening(particles.size());
    for(size_t i=0; i<particles.size(); i++)
        expandedSoftening[i] = softening.size()==1 ? softening[0] : softening[i];
    chooseGridRadiiSoftened(particles, expandedSoftening, gridSizeR, rmin, rmax);
    std::vector<double> gridRadii = math::createExpGrid(gridSizeR, rmin, rmax);
    if(isSpherical(sym))
        lmax = 0;
    if(isZRotSymmetric(sym))
