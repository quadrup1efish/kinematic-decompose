...
double erf(double xx)
{
    ...
    if(x >= 6)
        return xx>=0 ? 1 : -1;
    double x2 = x*x;
    if(x <= 1) {  // (4,5) Pade approximation, relative accuracy 2e-18
        return (xx>=0 ? x : -x) *
        (55588.3281628747661 + x2 * (7002.80663022352877 + x2 * (2231.85943452201646 + x2 *
        (90.0206014113792100 + x2 * 9.60474668359837293)))) /
        (49263.8731588434642 + x2 * (22627.3668423909950 + x2 * (4594.00222294555039 + x2 *
        (521.325431813055512 + x2 * (33.5602495188408435 + x2)))));
    } else {  // 1 < x < 6
        return (xx>=0 ? 1 : -1) * (1 - exp(-x2) *
...
double erfc(double xx)
{
    ...
    if(xx >= 27)
        return 0;  // underflow
    double x = xx>=0 ? xx : -xx;
    double val = exp(-x*x) *   // several choices of Pade approximation, depending on the range of x
    ( x <= 1.0 ?
        (143.864268811190206 + x * (170.045726357189856 + x * (102.764556737666806 + x *
        (34.9695209081232597 + x * (6.67201920046336468 + x * 0.563995633354177327))))) /
        (143.864268811190222 + x * (332.379170173159938 + x * (333.950019126688781 + x *
        (187.634891038629067 + x * (62.5463219575909556 + x * (11.8192335173109886 + x))))))
    : x <= 2.75 ?
        (60.1799598571714942 + x * (84.4044987270066112 + x * (58.0455797839876430 + x *
        (23.0271906339608483 + x * (5.25117277294393737 + x * 0.564186031906747693))))) /
        (60.1799711719308782 + x * (152.310210140737825 + x * (169.729828794577647 + x *
        (107.505430460066068 + x * (41.3178147355720918 + x * (9.30725653864402766 + x))))))
    : x <= 7.5 ?
        (18.8826356633383421 + x * (31.9987880616100095 + x * (25.9425959014533305 + x *
        (12.4682335534265815 + x * ( 3.5323571669327563 + x * 0.564189574924165163))))) /
        (18.8885333621146789 + x * (53.2778012226293742 + x * (67.2719636133038381 + x *
        (49.1120348570521633 + x * (22.5993962435309074 + x * (6.26093910573849456 + x))))))
    : /* x < 27 */
        (-0.3963519794505080 + x * (1.70414090833807094 + x * (1.47575169990170068 + x *
        (3.09421738074663801 + x * (0.68481959049029559 + x * 0.564189583553659917))))) /
        ((5.2625958555213430 + x * (3.22261116306737577 + x * (5.98435741157532766 + x *
        (1.21381112188192021 + x)))) * (x * x))
    );
    return xx >= 0 ? val : 2 - val;
}
